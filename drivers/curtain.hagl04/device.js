...
class AqaraCurtainB1 extends ZigBeeDevice {
  ...
  async initSettings() {
    try {
      const { xiaomiCurtainClearPosition, xiaomiCurtainReverse, xiaomiCurtainOpenCloseManual } = await this.zclNode.endpoints[this.getClusterEndpoint(XiaomiBasicCluster)].clusters[XiaomiBasicCluster.NAME].readAttributes(['xiaomiCurtainClearPosition', 'xiaomiCurtainReverse', 'xiaomiCurtainOpenCloseManual']).catch(this.error);
