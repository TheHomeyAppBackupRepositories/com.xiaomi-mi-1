...
const { localTimeToUTC, setTime } = require('../../lib/time');
...
const AqaraSpecificTimeBoundCluster = require('../../lib/AqaraSpecificTimeBoundCluster');
...
class AqaraE1TSmartRadiatorThermostat extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // this._externalTemperature(26);

    if (this.hasCapability('alarm_battery')) await this.removeCapability('alarm_battery');
    ...
    if (this.hasCapability('measure_temperature')) {
      this._thermostatCluster.on('attr.localTemperature', this.onTemperatureMeasuredAttributeReport.bind('thermostatCluster', 'localTemperature', this));
    }
    ...
    if (this.hasCapability('thermostat_mode_AqaraTRV')) {
      this.registerCapabilityListener('thermostat_mode_AqaraTRV', async thermostatMode => {
        ...
        if (thermostatMode !== currentThermostatMode) {
          this.log('Set Thermostat mode from ', currentThermostatMode, 'to:', thermostatMode, thermostatMode === 'off', currentThermostatMode === 'off', thermostatMode !== 'off');
    ...
    if (this.hasCapability('target_temperature')) {
      ...
      this.registerCapabilityListener('target_temperature',
        async (value, opts) => {
          ...
          if (thermostatMode === 'auto' || thermostatMode === 'off' || thermostatMode === 'away') {
  ...
  onAqaraAttributeReportLogger(attribute, value) {
    ...
    switch (attribute) {
      ...
      case 'aqaraTRVValveAlarm':
        this.log('handle report (cluster: AqaraManufacturerSpecificCluster, attribute: aqaraTRVValveAlarm, capability: alarm_valve), parsed payload:', value === 1);
        if (this.hasCapability('alarm_valve')) this.setCapabilityValue('alarm_valve', value === 1).catch(this.error);
        break;
      case 'aqaraTRVWindowOpen':
        this.log('handle report (cluster: AqaraManufacturerSpecificCluster, attribute: aqaraTRVWindowOpen, capability: alarm_contact), parsed payload:', value === 1);
        if (this.hasCapability('alarm_contact')) this.setCapabilityValue('alarm_contact', value === 1).catch(this.error);
        break;
  ...
  onPresetAttributeReport(reportingClusterName, reportingAttribute, value) {
    ...
    if (preset !== 'setup') {
      ...
      // this.log('aqaraTRVPreset: device in setup mode');
    }
  }
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    // thermostat_setpoint_away attribute
    if (changedKeys.includes('thermostat_auto_schedule_switch')) {
      attributes.aqaraTRVSchedule = newSettings.thermostat_auto_schedule_switch;
      this.log('SETTINGS | Write Attribute - Aqara Manufacturer Specific Cluster - aqaraTRVSchedule', newSettings.thermostat_auto_schedule_switch);
    }
    try {
      if (Object.keys(attributes).length > 0) {
        this.log('=>', attributes);
        // const payload = { aqaraTRVAwayPresetTemperature: value * 100 };
        // this.log('Setting target temperature', value, payload);
  ...
  onAqaraLifelineAttributeReport({
    ...
    // 104: alarm_valve
    if (typeof state4 === 'number') ;
    this.debug('onAqaraLifelineAttributeReport | state4', state4);
    // 105: battery
  ...
  async _externalTemperature(value) {
    ...
    await Util.wrapAsyncWithRetry(() => this.zclNode.endpoints[this.getClusterEndpoint(AqaraManufacturerSpecificCluster)].clusters[AqaraManufacturerSpecificCluster.NAME]
      .writeAttributes({ aqaraTRVSensorType: 1 }), 3);
